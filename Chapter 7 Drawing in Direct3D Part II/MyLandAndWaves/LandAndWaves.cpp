...
void LandAndWaves::OnInitialize() {
    ...
    CD3DX12_ROOT_SIGNATURE_DESC desc =
        {2, rootParams, 0, nullptr, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT};
    ...
    ThrowIfFailed(
        device_->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(pso_.GetAddressOf())));
...
void LandAndWaves::OnUpdate() {
    ...
    XMMATRIX proj =
        XMMatrixPerspectiveFovLH(0.25f * MathHelper::Pi, GetAspectRatio(), 1.0f, 1000.0f);
...
void LandAndWaves::BuildLandGeometry() {
    ...
    for (const auto& v : gridMesh.Vertices) {
        auto p = v.Position;
        p.y = height(p.x, p.z);
        vertices.emplace_back(p, color(p.y));
    }
...
void LandAndWaves::DrawAllRenderItems() {
    ...
    for (const auto& item : renderItems_) {
        ...
        auto address = objectCbuffer->GetElementGpuVirtualAddress(item.objectCbufferIndex);
